...
{
   mzlib::body2d sun;
   sun.mass_c.mass = mzlib::consts::sun_mass;
    
   mzlib::body2d earth;
   earth.mass_c.location = {0,mzlib::consts::earth_distance_sun};
   earth.mass_c.mass = mzlib::consts::earth_mass;
    
   universe.add(sun);
	universe.add(earth);
    
   mzlib::vector2d f_sun = universe.find(sun)->data.gravity;
   mzlib::vector2d f_earth = universe.find(earth)->data.gravity;
...
TEST_F(fixture_universe, sun_earth_month_travel_barneshut_with_quotient_more_than_0)
{
   ...
   earth.mass_c.location = earth_location_start;
   earth.mass_c.mass = mzlib::consts::earth_mass;
   ...
   mzlib::vector2d earth_location_quarter_later = universe.find(earth)->mass_c.location;
...
TEST_F(fixture_universe, long_earth_around_the_sun)
{
   ...
   mzlib::vector2d earth_location_year_later = universe.find(earth)->mass_c.location;
...
TEST_F(fixture_universe, moving_object)
{
   ...
   body.mass_c.location = {1.0_m, 2.0_m};
   ...
   // measure location
   mzlib::vector2d location_after_move = universe.find(body)->mass_c.location;
...
TEST_F(fixture_universe, moving_object_while_gravity_simulation_running)
{
   ...
   // setup start state   
   body1.mass_c.mass = 10.0_kg;
   body1.mass_c.location = body1_location_start;
   body2.mass_c.mass = 10.0_kg; 
   body2.mass_c.location = body2_location_start;
   ...
   // measure location
   mzlib::vector2d body1_location_without_move = universe.find(body1)->mass_c.location;
   ...
   // simulation   
   universe.forward_time(1.0_s, 1.0_s);
   mzlib::vector2d move_to = universe.find(body1)->mass_c.location + body1_move_vector;
   universe.move(body1, move_to);
   universe.forward_time(1.0_s, 1.0_s);
   
   // measure location
   mzlib::vector2d body1_location_with_move = universe.find(body1)->mass_c.location;
