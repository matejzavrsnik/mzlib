...
class fixture_space : public ::testing::Test 
{

protected:
   
   fixture_space() {}
   virtual ~fixture_space() {}
   virtual void SetUp() {}
...
TEST_F(fixture_space, set_gravitational_constant)
{
   space->set_gravitational_constant(0.5);
   ASSERT_DOUBLE_EQ(0.5, space->get_gravitational_constant());
}

TEST_F(fixture_space, big_ben_force_on_empire_state_building)
{
   mzlib::cbody big_ben;
...
TEST_F(fixture_space, sun_force_on_earth)
{
   mzlib::cbody sun;
   ...
   mzlib::math::cvector2d f_sun = space->find_body(sun)->data.force;
   mzlib::math::cvector2d f_earth = space->find_body(earth)->data.force;
    
   ASSERT_EQ(f_earth, -f_sun);
   ASSERT_NEAR(f_earth.length(), 3.53e+22, 0.01e+22);
}

TEST_F(fixture_space, long_earth_around_the_sun)
{
   mzlib::cbody sun;
   sun.mass = 1.989e30;
