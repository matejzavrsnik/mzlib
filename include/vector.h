...
public:
    ...
    template <typename... T> 
    constexpr cvector(T... val) : m_array{val...} {}
    
    constexpr cvector() : m_array{0} {}; // the rest will be zero
    ...
    cvector(const std::initializer_list<TYPE>& list) : m_array{0}
    {
    ...
    cvector(const std::vector<TYPE>& vec) : m_array{0}
    {
...
template<typename TYPE, size_t DIM>      // template declaration 
cvector<TYPE,DIM>                        // return type
    operator*(                           // operator
        const TYPE& scalar,              // left hand operand
        const cvector<TYPE,DIM>& vector) // right hand operand
{
