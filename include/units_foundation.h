...
namespace mzlib { namespace units {
...
class cquantity {
    ...
    protected:
        type value;
        constexpr cquantity(type v) : value(v) {}
    public:
        ...
        unit equals         (const unit& other  ) const { return unit(value -  other.value); }
        bool within_epsilon (const unit& epsilon) const { return std::fabs(value) < epsilon.value; }
        ...
        unit  operator += (const unit& other)       { return unit(value += other.value); }
        ...
        type  operator /  (const unit& other) const { return value / other.value; }
        // Can divide by unitless value, result is the same unit
        unit  operator /  (const type& other) const { return unit(value /  other); }
        unit  operator /= (const type& other)       { return unit(value /= other); }
    ...
    // Some operations have no physical meaning:
        ...
        //type operator /= (const unit& other) { return 0; }
            // Can't add and subtract values without unit. How much is 100m - 20 ?
        //unit operator += (const type& other) { return unit(0); }
        //unit operator +  (const type& other) { return unit(0); }
        //unit operator -= (const type& other) { return unit(0); }
        //unit operator -  (const type& other) { return unit(0); }
            // Valid operation, but it it will result in new quantity, so it needs to be defined later
        //unit operator *  (const unit& other) { return unit(0); }
            // Will result in new quantity which cannot be assigned to ol quantity
        //unit operator *= (const unit& other) { return unit(0); }
        //unit operator %  (const unit& other) { return unit(0); }
            // Comparing floating point values is dangerous
        //bool operator == (const unit& other) { return false;  };
        //bool operator != (const unit& other) { return false;  };
};

// Followed by macro to define every physical quantity
...
constexpr q_class operator"" _##q_short##_ (long double v) { return operator "" _##q_basic##_ ( v * (long double)q_factor ); } \
constexpr q_class operator"" _##q_short##_ (unsigned long long int v) { return operator "" _##q_basic##_ ( v * (long double)q_factor ); } 
