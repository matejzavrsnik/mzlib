...
namespace util
{
    ...
    inline unsigned int 
    get_random_integer_between(unsigned from, unsigned to)
    {
    ...
    public:
        ...
        void add_event(T event) {
            auto found = std::find_if(m_events.begin(), m_events.end(), [&](event_t& e){return e.first == event;});
    ...
    get_substring_between(const std::string& str, const std::string& start, const std::string& end)
    {
        ...
        if(pos_start != std::string::npos && pos_end  != std::string::npos) {
            if(pos_end < pos_start) std::swap(pos_start, pos_end);
